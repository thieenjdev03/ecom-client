...
- ProductVariantDto:
  ...
  - `color_id?` (string, UUID reference)
  - `size_id?` (string, UUID reference)
  - `barcode?` (string)
  - `image_url?` (string, URL format) – Hình ảnh riêng cho variant
...
- **Hình ảnh Variant** (Mới):
  - Mỗi variant có thể có `image_url` riêng để hiển thị hình ảnh đại diện cho biến thể đó.
  - `image_url` là optional, variant không có hình ảnh vẫn hợp lệ.
  - FE upload hình lên Cloudinary/CDN trước, sau đó gửi URL trong payload `variants[].image_url`.
  - Khi cập nhật variant, `image_url` được giữ nguyên nếu không có thay đổi.
...
Tạo product có variants (kèm `color_id`, `size_id`, `image_url`):
```json
{
  ...
  "variants": [
    {
      ...
      "price": 399000,
      "stock": 10,
      "image_url": "https://res.cloudinary.com/shop/image/upload/v1/polo-black.jpg"
    },
    {
      "name": "L - Blue",
      "color_id": "<color-uuid>",
      ...
      "price": 399000,
      "stock": 5,
      "image_url": "https://res.cloudinary.com/shop/image/upload/v1/polo-blue.jpg"
    }
  ],
  "category_id": 1
}
...
- `variants:ProductVariant[]` (jsonb, default [])
  - `ProductVariant = { name:string; color_id:string; size_id:string; sku:string; price:number; stock:number; barcode?:string; image_url?:string }`
  ...
  "variants": [
    {
      ...
      "price": 199000,
      "stock": 20,
      "image_url": "https://res.cloudinary.com/shop/image/upload/v1/tshirt-s-black.jpg"
    },
    {
      "name": "M / Black",
      "sku": "TSHIRT-M-BLK",
      ...
      "price": 199000,
      "stock": 30,
      "image_url": "https://res.cloudinary.com/shop/image/upload/v1/tshirt-m-black.jpg"
    }
  ],
  "status": "active"
}
